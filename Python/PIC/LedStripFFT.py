...
for i in range(11):
	if done==-1 and i<10:
		try:
			...
			print("ttyUSB{0} port erroné, test du suivant.".format(i))
	elif i==10:	
		...
		Label(ErrorScreen, text="FTDI non détecté").grid(row=0, column=0, columnspan=2, padx=25, pady=5)
		...
		print("ttyUSB{0} port connecté.".format(done))
		break
...
def pulseThread():
	...
	while stopPulseThread==0:
		...
		low=int(abs((fft[2*chunk/1024]+fft[3*chunk/1024]+fft[4*chunk/1024])*40/3))
		...
		med=int(abs((fft[99*chunk/1024]+fft[100*chunk/1024]+fft[101*chunk/1024])*180)/3)
		...
		high=int(abs((fft[199*chunk/1024]+fft[200*chunk/1024]+fft[201*chunk/1024])*270)/3)
